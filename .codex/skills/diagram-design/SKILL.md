---
name: diagram-design
description: Diagram grammar for the /slide pipeline. Use ONLY inside slide generation, when a slide needs a real diagram — architecture, flowchart, sequence, state machine, ER / data model, timeline, swimlane, quadrant, nested, tree, org chart, layer stack, venn, or pyramid — to author it as inline SVG inside the React slide component. Provides type-specific layout conventions, a complexity budget, node treatments, anti-patterns, and a taste gate. The visual skin is locked to the active design theme (currently Jangpm — Arial, single #4633E3 accent, monochrome, light) via references/style-guide.md and is re-skinned by /theme-init; never run website onboarding. Not a standalone HTML generator here. (Mermaid and Pencil batch_design remain the existing in-slide diagram paths; this provides the hand-authored inline-SVG grammar.)
license: MIT
metadata:
  version: "1.0"
---
...
Create visual diagrams as **inline SVG**, following an opinionated design system. Fourteen diagram types. One shared design system, complexity budget, and taste gate. Type-specific conventions live in `references/` and are loaded only when you pick a type.

> **⚠️ slide-pencil scope — read first.** In this repo this skill is **not** a standalone diagram generator and is **not** invoked on its own. It is the **hand-authored inline-SVG grammar used inside the `/slide` pipeline** when a slide needs a real diagram. Read it together with the `/slide` workflow; the diagram becomes inline SVG in `src/slides/Slide*.tsx`, not a standalone `.html` file.
>
> Two consequences override the generic instructions below:
> 1. **Skin is locked to the active theme (Jangpm).** Author every color against the theme tokens in [`references/style-guide.md`](references/style-guide.md) — `var(--accent)`, `var(--text)`, `var(--border)`, etc. — **not** the editorial hex values shown in the code samples. Font is Arial (`var(--font-sans)`); light mode only; one accent (`#4633E3`), no second hue. The hexes throughout this file and in `assets/*.html` are **illustrative of the old editorial skin only** — `style-guide.md` is the live source of truth and is re-skinned by `/theme-init`.
> 2. **No onboarding, no standalone HTML.** Skip §0 below — do not fetch a website or ask the user for brand colors. Skip the §11 "self-contained .html" instruction — the output is inline SVG embedded in the slide's JSX (see §11 note).

---

## 0. Skin is locked to the active theme — no setup gate

**This skill does not run the first-run style-guide gate or website onboarding in slide-pencil.** The skin is fixed to the active design theme (currently **Jangpm**) and lives in [`references/style-guide.md`](references/style-guide.md), already re-skinned to the theme tokens.

- **Do not** ask the user to pick brand colors, **do not** fetch a website, **do not** follow `references/onboarding.md` (kept only as upstream reference — inert here).
- To change the skin, the theme itself is swapped via **`/theme-init`**, which rewrites the `THEME:START … THEME:END` block in `style-guide.md` (replacement point #8). You never edit it by hand for theming.
- Always author colors as `var(--*)` tokens (see `style-guide.md`), so a future theme swap re-skins every diagram for free.
...
> **⚠️ slide-pencil: `assets/` is not shipped in this repo.** The standalone `template*.html` / `example-*.html` fixtures are gitignored (repo-wide `assets/` rule) and are the **old editorial skin** anyway — not the Jangpm skin you must use. Do **not** copy them. Instead, build the inline SVG directly from the matching `references/type-<name>.md` grammar + the Jangpm tokens in `references/style-guide.md` (see §11). The variant table and "copy a template" steps below describe upstream standalone use only; ignore them here. The minimal-light variant is the only relevant target (no dark, no full-editorial, no consultant skin).

Every diagram ships in three variants (see `assets/` — upstream only, not present here):
...
**In slide-pencil, the output is inline SVG inside the React slide component — not a standalone `.html` file.** Ignore the standalone-HTML instruction; it applies only to upstream standalone use.

- The diagram is `<svg>…</svg>` markup placed in the slide's JSX (`src/slides/Slide*.tsx`), inside the diagram slide's content area (e.g. full-bleed under the `.headline`, above the `.gm`).
- **No external CSS, no Google Fonts `<link>`, no `<script>`, no external images** — everything is inline SVG.
- **Colors = `var(--*)` theme tokens** (see `references/style-guide.md`), not hardcoded hex. **Font = inherited Arial** (`var(--font-sans)`); don't set a font family unless necessary.
- Keep the SVG responsive within the 1280×720 slide: set a `viewBox` and let it scale to the content area; respect the deck's Governing Message footer (don't overlap `.gm`).
- The `/slide` Step 6 PPTX pipeline rasterizes diagram SVGs to images — keep the SVG self-contained (no external refs) so that conversion works. See `../slide/references/pptx-build.md` (3-A: 차트/다이어그램 SVG → image).
