...
4. `.codex/skills/theme-init/references/theme-replacement-map.md` 로드하여 교체 대상 8개 지점 확인 (#7 = 이미지 프롬프트 팔레트 앵커, #8 = diagram-design style-guide.md 다이어그램 스킨)
...
11. **이미지 프롬프트 팔레트 앵커 교정 (HARD) — `README.md` + `.codex/skills/slide/SKILL.md` 둘 다:** illustration/diagram 프롬프트 줄(`muted pastel tones aligned with #4633E3 indigo accent` 형태)이 활성 테마에 고정돼 있다. **두 파일 모두** 새 테마 토큰으로 갱신:
   ...
   - **`.codex/skills/slide/SKILL.md` (THEME 블록 밖이라 마커로 안 잡힘):** Step 3.5 이미지 어댑터 표의 `| illustration |` / `| diagram |` 행 프롬프트(`muted pastel tones aligned with #4633E3 indigo accent` / `monochrome with a single #4633E3 indigo accent`)와 그 아래 예시 프롬프트 줄의 `#4633E3` / `indigo` / `muted pastel`도 위와 동일 규칙으로 교체. 스타일 락 보존.
