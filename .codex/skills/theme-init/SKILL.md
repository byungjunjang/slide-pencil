...
0. **활성 테마 자동 감지 (`<active-theme>` 확정, HARD)** — 이후 모든 from-theme 경로의 기준. `jangpm`을 가정하지 말 것:
   ...
   [ -z "$ACTIVE_THEME" ] && ACTIVE_THEME=$(basename "$(dirname "$(ls .codex/skills/slide/references/*/theme-rules.md | head -1)")")
...
4. `.codex/skills/theme-init/references/theme-replacement-map.md` 로드하여 교체 대상 8개 지점 확인 (#7 = README codex-image 팔레트 앵커, #8 = diagram-design style-guide.md 다이어그램 스킨)
...
2.5. **레퍼런스 덱(기존 HTML 슬라이드)이 제공되면** — `ingest-deck.mjs`로 레이아웃 디바이스 추출:
   ```bash
   node .codex/skills/theme-init/scripts/ingest-deck.mjs <deck.html|dir> --out output/_ingest.json
   ```
...
3. 추출 결과를 **토큰 컨트랙트 v1** 이름으로 매핑 (`.codex/skills/theme-init/references/theme-replacement-map.md`의 컨트랙트 섹션 참조):
   ...
   - Card chrome (card_style): `--card-bg, --card-border-color` — filled|hairline|borderless를 이 두 토큰으로 매핑 (`.codex/skills/theme-init/references/theme-replacement-map.md`의 card_style 표). `_slide.css .card` + `slide-system.tsx Card`가 참조
...
FILE 교체:
  ...
  · .codex/skills/slide/SKILL.md — THEME 블록
  · .codex/skills/slide/references/<active-theme>/  →  .../<new-theme>/
  · .codex/skills/slide/references/<new-theme>/colors_and_type.css (패턴 토큰 SSOT — src/index.css와 동일 값)
  · .codex/skills/slide/references/<new-theme>/patterns/*.html (Step 4.5 cover/closing/feature-board 재작곡)
  · .codex/skills/slide/references/<new-theme>/DESIGN.md (신규 — slide-plan 입력)
  · README.md — codex-image 일러스트 어댑터 팔레트 앵커 (#4633E3/indigo/pastel → 새 테마 accent/hue/무드)
  · .codex/skills/diagram-design/references/style-guide.md — 다이어그램 스킨 토큰 (THEME 블록 #8, src/index.css와 동일 값)
...
3. `.codex/skills/slide/SKILL.md` — Edit으로 THEME 마커 사이 내용 교체
4. **디렉토리 이동 + 경로 참조 일괄 치환 (스크립트):**
   ```bash
   node .codex/skills/theme-init/scripts/rename-theme.mjs <active-theme> <new-theme>   # 미리보기: --dry-run
   ```
   ...
   - **자동 제외:** `.codex/skills/theme-init/**`(제너레이터 canonical 예시)·`.pen`·node_modules/dist/output.
   - **theme-replacement-map.md 수동 갱신:** 이 맵은 `.codex/skills/theme-init/references/`(제외 1 zone) 안에 있어 rename-theme이 스캔/치환하지 않는다(테마 교체를 설명하는 canonical 예시 + 이력 보존). 맵의 운영 줄("현재 활성 테마" 섹션 등)은 Step 4 #12에서 수동 갱신한다.
...
6. **`references/<new-theme>/colors_and_type.css` 생성 + 클래스 패리티 (스크립트):**
   ```bash
   node .codex/skills/theme-init/scripts/gen-colors-and-type.mjs <new-theme>
   ```
...
7. `.codex/skills/slide/references/<new-theme>/patterns/` 내용 교체(옵션)
8. **`.pen` 시각 SSOT 생성** — 두 경로 자동 분기:
   ...
   **(b) 사용자가 `.pen` 미제공 (기본 경로)** — Pencil CLI로 자동 생성:
   ...
   ```bash
   git rm <active-theme>-design-system.pen   # <active-theme> = Step 0 감지 슬러그
   node .codex/skills/theme-init/scripts/pen-guard.mjs pre <new-theme>-design-system.pen   # 타깃 부재 보장 (truncate 방지)
   ( cat <<'PENCIL'
   ...
   **검증** (HARD RULE): `node .codex/skills/theme-init/scripts/pen-guard.mjs verify <new-theme>-design-system.pen`이 통과해야 한다(0바이트/부재면 비-0 exit + 진단). 0바이트면 (1) 타깃이 호출 전 잔존했거나(`rm -f` 누락), (2) `save()`와 `exit()` 사이 settle이 짧았던 것(`sleep 2` → `sleep 3~5`). 위 가트차 박스 순서대로 — 타깃 `rm -f` 보장 → `sleep` 상향 → 재실행. `../slide/references/pencil-cli.md` 단일 진실 원천 참조.
9. **내부 경로/파일명 참조 치환 — Step 4 #4의 `rename-theme.mjs`가 수행함.** 스크립트가 `references/<active-theme>` 디렉토리 참조와 `<active-theme>-design-system.pen` 파일명 문자열을 일괄 치환하며 다음 안전장치를 구현한다(별도 수동 `rg`/Edit 불필요):
   - **제외 1 — 제너레이터 canonical 예시:** `.codex/skills/theme-init/**`(이 SKILL.md 포함)는 치환하지 않는다 — 여기 등장하는 `references/jangpm`·`<active-theme>`는 from-theme를 설명하는 canonical 예시이지 운영 경로가 아니다.
   - **제외 2 — theme-replacement-map.md (제외 1에 편입):** 이 맵은 `.codex/skills/theme-init/references/theme-replacement-map.md`로 이동해 제외 1(theme-init/** 미스캔)에 포함된다. 맵은 운영/이력 참조가 섞여 있고 자기 `references/jangpm` 언급을 "동적 예시"로 명시하므로 자동 치환 대상이 아니다. 맵의 "현재 활성 테마" 섹션 등 운영 줄은 Step 4 #12에서 수동 갱신한다.
   - 루트 `.pen` 파일 자체(git rm/cp/생성)는 #8에서 처리하며, 여기선 문서 내 파일명 *문자열*만 치환한다.
10. (`<active-theme>-design-system.pen` 문자열 치환은 #9의 `rename-theme.mjs`가 함께 수행 — 별도 단계 불필요. `.codex/skills/theme-init/references/theme-replacement-map.md`는 제외 zone이라 #12에서 수동 갱신.)
11. **codex-image 일러스트 어댑터 팔레트 앵커 교정 (HARD) — `README.md` + `.codex/skills/slide/SKILL.md` 둘 다:** illustration/diagram 프롬프트 줄(`muted pastel tones aligned with #4633E3 indigo accent` 형태)이 활성 테마에 고정돼 있다. **두 파일 모두** 새 테마 토큰으로 갱신:
   ...
   - **`.codex/skills/slide/SKILL.md` (THEME 블록 밖이라 마커로 안 잡힘):** Step 3.5 codex-image 어댑터 표의 `| illustration |` / `| diagram |` 행 프롬프트(`muted pastel tones aligned with #4633E3 indigo accent` / `monochrome with a single #4633E3 indigo accent`)와 그 아래 예시 `/codex-image` 프롬프트 줄의 `#4633E3` / `indigo` / `muted pastel`도 위와 동일 규칙으로 교체. 스타일 락 보존.
   - **`.codex/skills/slide/references/image-archetypes.md` (5종 근거형 아키타입):** "테마 결합 토큰" 표와 각 아키타입 앵커의 `<ACCENT_HEX>`(=`#4633E3`) / `<ACCENT_HUE>`(=`indigo`) / `<MOOD>`(=`muted pastel / monochrome`)를 위와 동일 규칙으로 교체. 스타일 락 보존.
...
2. **레이아웃 토큰 설계** — 시그니처 구현에 필요한 토큰을 식별해 **추가**(교체 아님). 토큰 컨트랙트 v1을 깨지 않고 **테마-스코프 확장 그룹**으로 더한다. 예: `--navy`, `--brand-spectrum-1..n`, `--link`, `--on-dark`, `--cta` / `--cta-ink`, `.dot-*` 유틸. **반드시 THEME:START/END 마커 안**에, `patterns/_slide.css`(가 import하는 `colors_and_type.css`) **와** `src/index.css` 양쪽에 동일하게 기록 (패턴 프리뷰=빌드 일치).
   - v1 코어 토큰 이름은 그대로 고정·교차테마 공유. 레이아웃 토큰은 **테마별 자유·additive**이며 교차테마 공유 대상이 아니다 (`.codex/skills/theme-init/references/theme-replacement-map.md` "레이아웃 토큰" 절 참조).
...
7. **동기화** — 승인된 레이아웃 어휘를 Step 4.6 DESIGN.md §5(layout grammar)·§6(header/body/footer), `theme-rules.md`(커버 전략·레이아웃 어휘), `.codex/skills/theme-init/references/theme-replacement-map.md`에 반영.
...
0. **정적 검증 게이트 (스크립트, deck 불필요):**
   ```bash
   node .codex/skills/theme-init/scripts/validate-theme.mjs <new-theme>
...
2. 샘플 슬라이드 5종(가능하면 기존 `src/slides/SlideAgent01~05`) 스크린샷 캡처:
   ...
   - `.codex/skills/export-pdf/` 또는 `.codex/skills/export-pptx/` 기존 스크린샷 스크립트 재활용 가능
...
1. **미리보기 생성 (스크립트):**
   ```bash
   node .codex/skills/theme-init/scripts/gen-theme-preview.mjs <new-theme>
...
| 파일 | 로드 시점 |
|------|----------|
| `.codex/skills/theme-init/references/theme-replacement-map.md` | Step 0 (사전 안전장치) |
...
기존 jangpm DESIGN.md는 **수동 1회 작성**되었다 (`.codex/skills/slide/references/jangpm/DESIGN.md`). 자동 추출보다 정확도 우선 — 신규 테마부터 자동 모드 적용.
