...
> 이 파일은 `/theme-init`이 새 테마의 `theme-rules.md`를 생성할 때 사용하는 템플릿. `{{PLACEHOLDER}}` 섹션을 실제 값으로 치환한다. 구조는 Jangpm 원본(`.codex/skills/slide/references/jangpm/theme-rules.md`)을 따른다.
...
- **색상 팔레트**: 2~3 코어 + 중립색, 고대비 필수. {{COLOR_MODE — 예: "라이트 모드 전용"}}.
  ...
  - **하드코드 hex 금지.** 모든 색상은 `var(--*)` 토큰 참조. 토큰 이름 계약은 `.codex/skills/theme-init/references/theme-replacement-map.md` 참조
...
**6개 섹션 완결성 체크**: 이 템플릿은 `.codex/skills/slide/references/jangpm/theme-rules.md`의 6개 섹션 구조를 그대로 따른다. 누락 섹션이 있으면 `/slide` 스킬 Step 1 로드 시 해당 룰이 부재 → 슬라이드 품질 저하. 모든 섹션이 채워졌는지 Step 3 diff 미리보기에서 사용자 확인 필수.
