...
//
// 안전장치(HARD):
//   - 제외 1 (제너레이터 예시 + 교체 맵): `.codex/skills/theme-init/**` 는 손대지 않는다.
...
// theme-replacement-map.md가 .codex/skills/theme-init/references/ (제외 1 zone)로 이동해
...
function walk(root, dir, files) {
  for (const entry of readdirSync(dir)) {
    ...
    if (st.isDirectory()) {
      ...
      if (relNorm.startsWith('.codex/skills/theme-init')) continue
...
function main() {
  ...
  const oldDir = join(root, '.codex/skills/slide/references', opts.old)
  const tag = opts.dryRun ? '[dry-run] ' : ''

  // 1) 디렉토리 git mv
  if (existsSync(oldDir)) {
    const fromRel = `.codex/skills/slide/references/${opts.old}`
    const toRel = `.codex/skills/slide/references/${opts.next}`
  ...
  const themeDirPrefixes = [
    `.codex/skills/slide/references/${opts.old}/`,
    `.codex/skills/slide/references/${opts.next}/`,
  ]
  const bareHits = []
  for (const f of files) {
