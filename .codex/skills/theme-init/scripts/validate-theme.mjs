...
//
// 검사 항목:
//   1. THEME 마커 존재 — src/index.css, CLAUDE.md, .codex/skills/slide/SKILL.md
...
function main() {
  ...
  const slideSkillPath = join(root, '.codex/skills/slide/SKILL.md')
  const mirrorPath = join(root, '.codex/skills/slide/references', theme, 'colors_and_type.css')
  ...
  if (!staleHexList) {
    ...
    const promptFiles = [
      ...
      ['image-archetypes.md', join(root, '.codex/skills/slide/references/image-archetypes.md')],
    ]
    const offenders = []
  ...
  {
    const themeRulesPath = join(root, '.codex/skills/slide/references', theme, 'theme-rules.md')
    const docs = [
