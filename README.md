...
A. 안 됩니다. `/slide` 스킬은 정책상 **Pencil 프레임 수 == TSX 파일 수 == 계획 슬라이드 수**가 정확히 일치해야 진행합니다. CLI가 없거나 인증이 끊겨 있으면 즉시 파이프라인을 중단하고 사용자에게 경고합니다 — 이게 React-only 우회로 품질이 무너지는 것을 막는 핵심 장치입니다. `pencil login`으로 인증을 복구한 뒤 다시 시도하세요.
